...
seed=paste("p4s", "-", k, ".png", sep="")
png(seed)
par(mar = c(0,0,0,0))
image(rotate(zona), col=rainbow(k+1), xaxt='n', yaxt='n')
graphics.off()

cell=paste("p4c", "-", k, ".png", sep="")
png(cell)
par(mar = c(0,0,0,0))
...
write_xlsx(datos, "tarea4datos.xlsx")
  ...
  ggplot(data = stack(tarea4datos), aes(x = ind, y = values)) +
    stat_boxplot(geom = "errorbar", width = 0.9) + 
    geom_boxplot(fill = "#F8766D", colour = "#1F3552")+ 
    labs(x = "Número de semillas", y = "Distancia Mahattan")+ 
...
cinco<- c(2, 2, 8, 8, 8, 2, 3, 8, 3, 3, 8, 8, 3, 2, 3)
